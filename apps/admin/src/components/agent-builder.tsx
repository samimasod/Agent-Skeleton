...
export function AgentBuilderView({ agentId: initialAgentId }: AgentBuilderViewProps) {
  const [agentsList, setAgentsList] = useState<Agent[]>([])
  const [selectedAgentId, setSelectedAgentId] = useState<number>(initialAgentId || 1)
  ...
  const loadAgentsListAndModels = async () => {
    setIsLoading(true)
    setError("")
    try {
      const [agentsRes, modelsData, toolsData] = await Promise.all([
        agentsApi.list(1, 1, 50).catch(() => ({ agents: [], total: 0 })),
        llmApi.models().catch(() => []),
        agentsApi.listAvailableTools().catch(() => []),
      ])
      ...
      const targetId = initialAgentId || (agentsRes.agents.length > 0 ? agentsRes.agents[0].id : 1)
      setSelectedAgentId(targetId)

      if (agentsRes.agents.length > 0) {
        const found = agentsRes.agents.find((a: Agent) => a.id === targetId) || agentsRes.agents[0]
        loadSingleAgentConfig(found.id)
      } else {
        setIsLoading(false)
      }
    } catch (err: any) {
  ...
  const loadSingleAgentConfig = async (idToLoad: number) => {
    setIsLoading(true)
    try {
      const agentData = await agentsApi.get(idToLoad)
      setAgent(agentData)
  ...
  useEffect(() => {
    void loadAgentsListAndModels()
  }, [])

  const handleSelectAgent = (idStr: string) => {
    const newId = parseInt(idStr, 10)
    setSelectedAgentId(newId)
    void loadSingleAgentConfig(newId)
  }
  ...
  const handleSaveSettings = async () => {
    ...
    setError("")
    try {
      const updated = await agentsApi.update(selectedAgentId, {
        name,
      ...
      setAgent(updated)
      setAgentsList((prev) => prev.map((a) => a.id === selectedAgentId ? updated : a))
      setSaveSuccess(true)
  ...
  const handleCreateAgent = async () => {
    try {
      const created = await agentsApi.create({
        name: "New Admin Agent",
        description: "SuperAdmin Assistant Profile",
        system_prompt: "You are a helpful admin assistant.",
        model_id: "google/gemini-2.0-flash-001",
        temperature: 0.7,
        organization_id: 1,
      })
      setAgentsList((prev) => [created, ...prev])
      setSelectedAgentId(created.id)
      void loadSingleAgentConfig(created.id)
    } catch (err: any) {
      alert(err.message || "Failed to create new agent")
    }
  }
    ...
    <div className="space-y-4">
      ...
      <div className="flex items-center justify-between gap-4 border-b border-border pb-3">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="text-xl font-extrabold tracking-tight">Agent Builder & Playground</h1>
            <p className="text-xs text-muted-foreground">Configure AI personas, models, tools, and test streaming turns.</p>
          </div>
        </div>

        {/* Agent Picker & New Agent Button */}
        <div className="flex items-center gap-2">
          {agentsList.length > 0 && (
            <Select value={String(selectedAgentId)} onValueChange={handleSelectAgent}>
              <SelectTrigger className="w-56 h-9 text-xs font-semibold">
          ...
          <Button size="sm" onClick={handleCreateAgent} className="h-9 text-xs gap-1.5 font-bold">
            <Plus className="h-3.5 w-3.5" /> New Agent
          </Button>
        </div>
      </div>

      <div className="grid gap-4 lg:grid-cols-[1fr_1.3fr] h-[calc(100vh-180px)]">
        ...
        <Card className="flex flex-col h-full overflow-hidden border border-border">
          <CardHeader className="pb-3 border-b border-border/50 bg-card/50">
            ...
            <CardDescription className="text-xs">Adjust model weights, persona prompt, and capabilities.</CardDescription>
          </CardHeader>
          <CardContent className="flex-1 overflow-y-auto space-y-4 py-4 scrollbar-thin">
            ...
            <div className="grid gap-2">
              ...
              <Textarea
                ...
                className="h-36 text-xs font-mono resize-none leading-relaxed"
              />
            </div>
            ...
            <div className="space-y-3 pt-2 border-t border-border/50">
              <Label className="text-xs font-bold uppercase tracking-wider text-muted-foreground">Attach Custom Tools</Label>
                ...
                return (
                  <div
                    ...
                    className={`flex items-start space-x-3 p-2.5 rounded-lg border text-xs transition-colors ${
                      isSelected ? "border-primary/40 bg-primary/5" : "border-border/50 bg-card/50"
                    }`}
                  >
                    <Checkbox
                      id={`tool-${tool.name}`}
          ...
          <div className="p-3 border-t border-border/50 bg-card/50 flex items-center justify-between">
            {saveSuccess ? (
              <span className="text-xs text-[var(--positive)] font-semibold flex items-center gap-1">
                ✓ Saved successfully
              </span>
            ) : <span />}
            <Button onClick={handleSaveSettings} disabled={isSaving} size="sm" className="ml-auto font-bold">
        ...
        <AgentChatView
          ...
          agentName={agent?.name || `Agent #${selectedAgentId}`}
