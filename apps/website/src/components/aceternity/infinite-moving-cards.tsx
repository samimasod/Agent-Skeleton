  ...
  return (
    ...
    >
      ...
      >
        {items.map((item, idx) => (
          <li
            className="w-[350px] max-w-full relative rounded-2xl border border-white/10 flex-shrink-0 px-8 py-6 md:w-[450px] bg-gradient-to-b from-[#161f34]/80 to-[#0d1322]/80 backdrop-blur-md"
            key={idx}
          >
            <blockquote>
              <div
              ...
              <div className="relative z-20 mt-6 flex flex-row items-center gap-3">
                <div className="flex h-10 w-10 items-center justify-center rounded-full bg-[#006ddd]/20 text-[#7fc8ff] font-mono font-semibold border border-[#006ddd]/30">
                  {item.name.charAt(0)}
                </div>
                <span className="flex flex-col gap-1">
                  ...
                  <span className="text-xs leading-[1.6] text-gray-400 font-normal">
                    {item.title} • <span className="text-[#7fc8ff]">{item.company}</span>
                  </span>
                </span>
              </div>
            </blockquote>
